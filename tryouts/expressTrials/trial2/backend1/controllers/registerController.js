...
const registerController = async (req, res, next) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ userInfo: { username: result.username, roles } }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "5m" });
        //* send refresh-token to the client as http-only cookie.
        res.cookie('jwt', result.refreshToken, {httpOnly: true, maxAge: 10*60*1000}); 
